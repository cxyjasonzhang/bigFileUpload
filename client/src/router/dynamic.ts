...
const FallbackComponent = defineComponent({
  ...
  setup() {
    return () =>
      h('div', { style: 'padding: 40px; text-align: center; color: #999;' }, '页面组件未找到')
  },
})
...
function resolveComponent(component?: string) {
  ...
  const normalized = component?.replace(/^\/+|\/+$/g, '').replace(/\.vue$/, '')
  ...
  const walk = (nodes: MenuTree[]) => {
    nodes.forEach((node) => {
      ...
      if (node.menuType === 1) {
        ...
        const routeName = node.component?.replace(/[/.]/g, '-') || `menu-${node.menuId}`
        routes.push({
