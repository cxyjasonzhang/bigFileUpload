...
const menuTypeText = computed(() =>
  form.menuType === 0 ? '目录' : form.menuType === 1 ? '菜单' : '按钮',
)
...
watch(
  ...
  (type) => {
    ...
    rules.component = [
      { required: type === 1, message: '菜单类型必须填写组件地址', trigger: 'blur' },
    ]
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const message =
      props.dialogType === 'add' ? `新增${menuTypeText.value}成功` : `编辑${menuTypeText.value}成功`
