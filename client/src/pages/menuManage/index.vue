    ...
    <ElCard class="flex-1 art-table-card">
      ...
      >
        ...
        <template #menuType="{ row }">
          <ElTag :type="row.menuType === 0 ? 'warning' : row.menuType === 1 ? 'primary' : 'info'" disable-transitions>
...
<script setup lang="ts">
  ...
  const filterMenuTree = (nodes: MenuTreeNode[], keyword: string, status: string): MenuTreeNode[] => {
    ...
    for (const node of nodes) {
      const selfMatched =
        (!keyword || node.menuName.includes(keyword)) && (status === '' || node.status === Number(status))
      if (selfMatched) {
        result.push(node)
      } else {
