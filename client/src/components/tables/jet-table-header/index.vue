...
<template>
  <div id="art-table-header" class="flex-cb max-md:!block">
    <div class="flex-wrap">
      <slot name="left"></slot>
    </div>
    ...
    <div class="flex-c md:justify-end max-md:mt-3 max-sm:!hidden">
      ...
      <ElDropdown v-if="shouldShow('size')" @command="handleTableSizeChange">
        <div class="button">
          <SvgIcon name="sys/setting" :size="16" />
        </div>
        <template #dropdown>
          <ElDropdownMenu>
            <div
          ...
          <ElScrollbar max-height="380px">
            ...
            >
              ...
              >
                ...
                >
                  <SvgIcon
                    :name="item.fixed ? 'sys/unpin-line' : 'sys/drag'"
                    :size="16"
                    class="text-base"
                  />
                </div>
...
defineOptions({ name: 'JetTableHeader' })
