...
import { calculateFileHash, createFileChunks, type FileChunk } from './file'
import { checkFile, uploadChunk, mergeChunks, type CheckFileResult } from './api'
...
export class UploadQueue {
  ...
  matchTaskByHash(fileHash: string): UploadTask | undefined {
    return this.tasks.find((t) => t.fileHash === fileHash && t.status !== TASK_STATUS.SUCCESS)
  }
  ...
  loadTasks(): void {
    try {
      ...
      parsed.forEach((t) => {
        ...
        if (t.status === TASK_STATUS.UPLOADING || t.status === TASK_STATUS.HASHING) {
  ...
  async syncTaskWithServer(taskId: string): Promise<void> {
    ...
    try {
      const { data: checkResult } = await checkFile(task.fileHash, task.fileName)
    ...
    try {
      ...
      if (!task.fileHash) {
        ...
        const duplicate = this.tasks.find(
          (t) => t.id !== taskId && t.fileHash === hash && t.status !== TASK_STATUS.ERROR,
        )
        if (duplicate) {
          this.removeTask(taskId)
          this.currentTaskId = null
          throw new Error(`文件 "${task.fileName}" 与 "${duplicate.fileName}" 重复`)
        }
      }
      ...
      // 3. 记录已上传分片
      const existingChunks = (checkResult.data as CheckFileResult).uploadedChunks || []
      ...
      this.updateTask(taskId, {
        ...
        uploadProgress: Math.round((existingChunks.length / task.totalChunks) * 100),
      })
      this.currentTaskId = taskId
          ...
          try {
            ...
            if (curTask && curTask.status !== TASK_STATUS.PAUSED) {
              ...
              this.updateTask(taskId, {
                ...
                uploadProgress: Math.round((newUploaded / curTask.totalChunks) * 100),
              })
            }
          } catch (err) {
            ...
            if (retryCount >= MAX_RETRIES) {
              throw new Error(`分片 ${chunk.index} 上传失败，已重试 ${MAX_RETRIES} 次`)
            }
  ...
  pauseTask(taskId: string): void {
    const task = this.getTask(taskId)
    if (!task) return
    if (task.status === TASK_STATUS.UPLOADING || task.status === TASK_STATUS.HASHING) {
  ...
  async resumeTask(taskId: string, file?: File): Promise<void> {
    ...
    if (file) {
      ...
      if (!task.fileHash || task.fileName !== file.name || task.fileSize !== file.size) {
        this.updateTask(taskId, {
          fileHash: '',
          hashProgress: 0,
          fileName: file.name,
