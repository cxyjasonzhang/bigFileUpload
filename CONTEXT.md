...
| **退出登录清理** | `logout()` 除清登录态外，还须清空 `permission` 菜单树、`layout` 的历史 Tab（visitedRoutes）与动态注册的路由，防止跨用户残留导致越权访问（见 ADR-0010） |
...
| **用户分配角色** | 用户管理的新增/编辑弹窗中可为用户勾选角色（非必填，不含 R_SUPER）；保存时全量覆盖用户角色，但强制保留用户已有的超级管理员角色（R_SUPER），避免误清导致系统失去超管 |
